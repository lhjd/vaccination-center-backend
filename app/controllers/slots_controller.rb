class SlotsController < ApplicationController
  ...
  def index
    if params[:center_id]
      slots = Slot.where(center_id: params[:center_id])
      @slots = slots.reject { |s| s.bookings.count == s.capacity }
    else
      @slots = Slot.all
    end
  end
  ...
  def slot_params
    params.require(:slot).permit(:date, :start_time, :center_id, :capacity)
  end
end
